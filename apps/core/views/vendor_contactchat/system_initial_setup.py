...
=========================
        ...
        return render(request, "vendor_contactchat/system_initial_setup_detail.html", context)
...
def new(request):
    ...
    if in_group(request.user, "system"):
        ...
        while (True):
            ...
            contactchat_access_url_part = "cc" + random_character(20)
            contactchat_access_token = random_character(20)
            ...
            vendor_cc = Vendor.objects.filter(contactchat_access_url_part=contactchat_access_url_part).first()
            if (not vendor_fb) and (not vendor_ln) and (not vendor_cc):
                break
        ...
        context = {
            ...
            "contactchat_access_url_part": contactchat_access_url_part,
            "contactchat_access_token": contactchat_access_token,
            "oem_admin_flg": oem_admin_flg,
            "namespace": namespace
        }

        return render(request, "vendor_contactchat/system_initial_setup_new.html", context)
    ...
    if in_group(request.user, "system"):
        try:
            with transaction.atomic():
                ...
                contactchat_access_url_part = request.POST["contactchat_access_url_part"]
                contactchat_access_token = request.POST["contactchat_access_token"]
                ...
                vendor.contactchat_access_url_part = contactchat_access_url_part
                vendor.contactchat_access_token = contactchat_access_token
...
def copy_message_sequence(from_vendor_id, to_vendor):
    ...
    message_blocks = MessageBlock.objects.filter(vendor_branch_id=from_vendor_id)
